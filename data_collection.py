...
REVENUE_VARIATION_WITH_ROUTE_ADDITION_PATH = (
    "./revenue_variation_with_adding_routes.json"
)


def init_json(filepath, commodities, metro_lines):
    data = {"commodities": commodities, "edges": metro_lines, "experiments": []}
...
def append_revenue(k, revenue):
    ...
    data["experiments"].append({"k": k, "revenue": revenue})
...
def append_revenue_route_json(order_id, k, src, dst, revenue):
    ...
    if k == 1:
        exp = {"order_id": order_id, "steps": [], "final_revenue": None}
    ...
    for e in data["experiments"]:
        if e["order_id"] == order_id:
            ...
            break
    exp["steps"].append(
        {"step": k, "added_route": {"src": src, "dst": dst}, "revenue": revenue}
    )
...
def revenue_variation_with_adding_routes():
    ...
    routes = [
        ["Rabinder Sarobar", "Kavi Subhash", "blue", 10],
